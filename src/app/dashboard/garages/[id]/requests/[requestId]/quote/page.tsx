...
export default function WriteQuotePage() {
  const { id: garageId, requestId } = useParams<{ id: string; requestId: string }>();
  const router = useRouter();

  const garage = useOwnedGarage(garageId);
    ...
    submit.mutate(
      { requestId, garageId: garage.data.id, data: parsed.data },
      {
        onSuccess: () => {
          ...
          router.push(`/dashboard/garages/${garageId}?quoted=1`);
        },
  ...
  return (
    <section className="py-8">
      <Link href={`/dashboard/garages/${garageId}`} className="text-sm text-paper/60 hover:text-volt-bright">
        &larr; Request feed
      </Link>
      <h2 className="mt-2 font-display text-2xl font-bold">Write a quote</h2>
      ...
      {isActive && request.data && (
        <>
          <section className="mt-6 rounded-hex border border-ink-line bg-ink-soft p-6">
            <h3 className="font-display text-lg font-semibold">{request.data.title}</h3>
          ...
          <form onSubmit={onSubmit} className="mt-8 grid gap-8 lg:grid-cols-[1fr_280px]" noValidate>
            <div className="space-y-6">
              <section>
                <h3 className="font-display text-xl font-semibold">Line items</h3>
            ...
            <aside className="h-fit rounded-hex border border-ink-line bg-ink-soft p-6 font-mono text-sm lg:sticky lg:top-20">
              <h3 className="font-display text-lg font-semibold">Quote preview</h3>
